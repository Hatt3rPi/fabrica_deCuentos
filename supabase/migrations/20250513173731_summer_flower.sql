...
DROP FUNCTION IF EXISTS create_reset_token(text);
DROP FUNCTION IF EXISTS generate_reset_token();
...
CREATE FUNCTION generate_reset_token()
RETURNS text
LANGUAGE plpgsql
AS $$
DECLARE
...
CREATE FUNCTION create_reset_token(user_email text)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
CREATE FUNCTION reset_password(p_token text, p_new_password text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
