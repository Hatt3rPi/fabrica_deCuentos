    ...
    const loadPrompts = async () => {
      ...
      try {
        const response = await fetch(
          `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/get-prompt-secrets`,
          {
            headers: {
        ...
        if (!response.ok) {
          throw new Error('Error fetching prompt secrets');
        }

        const data = await response.json();
        setThumbnailPrompt(data.PROMPT_CREAR_MINIATURA_PERSONAJE || '');
        setDescriptionPrompt(data.PROMPT_DESCRIPCION_PERSONAJE || '');
      } catch (err) {
