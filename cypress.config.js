...
export default defineConfig({
  ...
  e2e: {
    ...
    async setupNodeEvents(on, config) {
      ...
      config.env = {
        ...config.env,
        VITE_SUPABASE_URL: process.env.VITE_SUPABASE_URL || 'https://ogegdctdniijmublbmgy.supabase.co',
      ...
      if (!process.env.VITE_SUPABASE_SERVICE_ROLE_KEY) {
        console.warn('⚠️  Advertencia: Falta la variable de entorno VITE_SUPABASE_SERVICE_ROLE_KEY');
        console.warn('   Algunas funciones de limpieza pueden no funcionar correctamente');
      }
      
      // Advertencia si no se proporciona la clave de la API de limpieza
      if (!process.env.CLEANUP_API_KEY) {
        console.warn('⚠️  Advertencia: Falta la variable de entorno CLEANUP_API_KEY');
        console.warn('   La función de limpieza de historias no funcionará correctamente');
      }
      
      // Asegurarse de que las variables de entorno requeridas estén presentes
      if (!config.env.VITE_SUPABASE_ANON_KEY) {
        console.warn('⚠️  Advertencia: Falta la variable de entorno VITE_SUPABASE_ANON_KEY');
      }
